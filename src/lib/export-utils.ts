...
function renderChartSVG(chart: SlideChartElement): string {
  ...
  if (chartType === 'pie' || chartType === 'donut') {
    ...
    if (showLegend) {
      data.labels.forEach((label, i) => {
        ...
        legend += `<text x="22" y="${187 - (data.labels.length - i - 1) * 12}" fill="#94a3b8" font-size="7">${label}</text>`
      })
    }
    return `<svg viewBox="0 0 200 200" style="width:100%;height:100%">${title ? `<text x="100" y="16" text-anchor="middle" fill="#cbd5e1" font-size="10" font-weight="600">${title}</text>` : ''}${paths}${legend}</svg>`
  }
  ...
  let svg = ''
  if (title)
    svg += `<text x="100" y="14" text-anchor="middle" fill="#cbd5e1" font-size="10" font-weight="600">${title}</text>`
  ...
  data.labels.forEach((label, i) => {
    svg += `<text x="${padding.left + barGroupW * i + barGroupW / 2}" y="${200 - padding.bottom + 12}" text-anchor="middle" fill="#94a3b8" font-size="6">${label.length > 6 ? label.slice(0, 6) + '..' : label}</text>`
  })
  ...
  if (showLegend && data.datasets.length > 1) {
    data.datasets.forEach((ds, i) => {
      ...
      svg += `<text x="${padding.left + i * 50 + 9}" y="${padding.top - 5}" fill="#94a3b8" font-size="6">${ds.label}</text>`
    })
  }
    ...
    .map((slide, i) => {
      ...
      const elements = sortedElements
        .map((el) => {
          ...
          return `<div style="position:absolute;left:${el.x}%;top:${el.y}%;width:${el.w}%;color:${color};font-size:${el.fontSize}px;font-weight:${el.fontWeight};text-align:${el.align};${lineH}white-space:pre-wrap;${el.fontFamily ? `font-family:${el.fontFamily};` : ''}${letterSp}${txtShadow}${trans}${filter}${zIndex}">${el.content}</div>`
        })
        .join('\n')
...
export function exportDocToHTML(doc: DocDocument): string {
  ...
  const content = doc.blocks
    .map((b) => {
      ...
      switch (b.type) {
        case 'h1':
          return `<h1${style}>${escapeHtml(b.content)}</h1>`
        case 'h2':
          return `<h2${style}>${escapeHtml(b.content)}</h2>`
        case 'h3':
          return `<h3${style}>${escapeHtml(b.content)}</h3>`
        case 'paragraph':
          return `<p${style}>${escapeHtml(b.content)}</p>`
        case 'bullet':
          return `<ul${style}><li>${escapeHtml(b.content)}</li></ul>`
        case 'numbered':
          return `<ol${style}><li>${escapeHtml(b.content)}</li></ol>`
        case 'quote':
          return `<blockquote${style}>${escapeHtml(b.content)}</blockquote>`
        case 'divider':
          return `<hr />`
        case 'page-break':
        ...
        case 'code':
          return `<pre style="background:#1e293b;color:#6ee7b7;padding:16px;border-radius:8px;overflow-x:auto;font-family:monospace"><code>${escapeHtml(b.content)}</code></pre>`
        case 'callout': {
          ...
          return `<div style="border-left:4px solid ${colors[v]};background:${colors[v]}10;padding:12px 16px;border-radius:4px;margin:1em 0">${icons[v]} ${escapeHtml(b.content)}</div>`
        }
        case 'checklist':
          return `<div style="display:flex;align-items:center;gap:8px;margin:4px 0"><input type="checkbox"${b.checked ? ' checked' : ''} disabled /><span${b.checked ? ' style="text-decoration:line-through;color:#94a3b8"' : ''}>${escapeHtml(b.content)}</span></div>`
        case 'footnote': {
          footnoteCount++
          return `<div style="font-size:0.85em;color:#64748b;border-left:2px solid #475569;padding-left:12px;margin:8px 0"><sup>[${footnoteCount}]</sup> ${escapeHtml(b.content)}</div>`
        }
        case 'math':
          return `<div style="text-align:center;font-size:1.3em;font-family:serif;margin:1em 0;padding:8px">${escapeHtml(b.content)}</div>`
        case 'table': {
          try {
        ...
        case 'textbox': {
          ...
          return `<div style="border:${tbBorder};background:${tbBg};border-radius:${tbRadius};box-shadow:${tbShadow};padding:${tbPadding};margin:1em 0;${b.data?.extra || ''}">${b.content}</div>`
        }
        case 'columns': {
          try {
            const cols: string[] = JSON.parse(b.content)
            return `<div style="display:flex;gap:24px;margin:1em 0">${cols.map((c) => `<div style="flex:1">${c}</div>`).join('')}</div>`
          } catch {
            return ''
          }
        }
        ...
        case 'drawing':
          return '<div style="text-align:center;padding:2em;color:#94a3b8">[描画コンテンツ]</div>'
        default:
          return `<p${style}>${escapeHtml(b.content)}</p>`
      }
    })
