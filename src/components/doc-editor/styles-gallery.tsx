...
interface Props {
  ...
  onBatchChangeType?: (type: BlockType) => void
}
...
const STYLES: { type: BlockType; label: string; preview: string }[] = [
  ...
  { type: 'h4', label: '見出し 4', preview: 'text-sm font-bold text-white' },
  { type: 'h5', label: '見出し 5', preview: 'text-xs font-bold text-white' },
  { type: 'h6', label: '見出し 6', preview: 'text-xs font-bold uppercase tracking-wide text-slate-200' },
...
export default function StylesGallery({ currentType, onChangeType, onBatchChangeType }: Props) {
  return (
    <div className="flex items-center gap-1 overflow-x-auto py-1 px-2 no-scrollbar">
      {STYLES.map((s) => (
        <button
          key={s.type}
          onClick={(e) => {
            if (e.shiftKey && onBatchChangeType) {
              onBatchChangeType(s.type)
            } else {
              onChangeType(s.type)
            }
          }}
          ...
          title={`${s.label}${onBatchChangeType ? ' (Shift+クリックで連続ブロックに一括適用)' : ''}`}
        >
          <span className={s.preview}>{s.label}</span>
        </button>
      ))}
      {onBatchChangeType && (
        <span className="flex-shrink-0 text-[9px] text-slate-600 ml-1">Shift+クリックで一括</span>
      )}
    </div>
  )
}
