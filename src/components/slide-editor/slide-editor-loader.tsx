...
import { getSlideDoc } from '@/lib/storage/cloud-store'
import { normalizeSlidesDoc } from '@/lib/storage/validate-doc'
...
export default function SlideEditorLoader({ id }: { id: string }) {
  ...
  useEffect(() => {
    let cancelled = false
    // Hard safety net: even if getSlideDoc hangs, stop the spinner after 12s
    ...
    getSlideDoc(id)
      .then((d) => {
        if (cancelled) return
        const safe = normalizeSlidesDoc(d)
        if (!safe) {
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-950">
        <div className="text-center max-w-sm px-6">
          <p className="text-slate-300 text-sm mb-4">スライドの読み込みに時間がかかっています。接続を確認してもう一度お試しください。</p>
